...
function getNodeBandwidthStatData(node_id,timeval)
{
	...
	var postUrl = "request_stat_data.php?mod=realtime&action=bandwidth_max"+"&node_id=" + node_id +"&timeval="+timeval;
...
function OnSelectNode()
{
	...
	window.location.href="stat_node_bandwidth_max.php?id="+txtSelectNode+"&date2="+nDateSelect2;
}
    ...
    <td valign="top" background="../images/content-bg.gif">
	   <table width="100%" height="31" border="0" cellpadding="0" cellspacing="0" class="left_topbg" id="table2" >
			<tr height="31" class="TabToolTitle">
				...
				<td height="31" width="85"><a href="stat_node_bandwidth_max.php?id=<?php echo $node_id; ?>"><span class="title_bt">峰值带宽</span></a></td>
...
<?php  
	...
	if($db_link)
	{
		...
		$sql = "SELECT max(bandwidth_down),max(bandwidth_up),avg(bandwidth_down),avg(bandwidth_up) FROM realtime_list_max WHERE node_id=$node_id AND time>=$timeval";
		...
		$sql = "SELECT max(bandwidth_down),max(bandwidth_up) FROM realtime_list_max WHERE node_id=$node_id AND time>=$timeval";
		$result2 = mysql_query($sql,$db_link);
		if($result2 && mysql_num_rows($result2)>0)
		{
...
?>
		...
		<table width="940" border="0" cellspacing="0" cellpadding="0" style="padding-left:45px">
			...
			<tr>
				...
				<td class="title_line" width="18%" height="25" align="right" style="background-color: whitesmoke"><strong>7天内最大下载带宽：</strong></td>
			...
			<tr>
				...
				<td class="title_line" width="18%" height="25" align="right" style="background-color: whitesmoke"><strong>7天内最大上传带宽：</strong></td>
	...
	<table width="800" border="0" class="bottom_btn">
			...
			<td>
			<span class="input_tips_txt3"><?php echo $show_this_name; ?> - 峰值带宽详细统计数据</span>
		...
		<div id="div_search_result2">
			<table width="800" border="0" class="dataintable" id="domain_table">
				<tr id="tr_domain_title">
					...
					<th align="right" width="120">每小时用户下载数据</th>
					<th align="right" width="120">每小时用户上传数据</th>
					<th align="right" width="120">用户下载峰值带宽</th>
					<th align="right" width="100" align="center">用户上传峰值带宽</th>
					<th align="right" width="100">源站上传数据</th>
					<th align="right" width="100">源站下载数据</th>
					<th align="right" width="100">源站上传峰值带宽</th>
					<th align="right" width="100" align="center">源站下载峰值带宽</th>					
				</tr>	
<?php				
			if($db_link){
				...
				$sql = "SELECT * FROM realtime_list_max where node_id='$node_id' AND time>='$timeval1' AND time<'$timeval2'"; 
				$result = mysql_query($sql,$db_link);
				if($result){
					$row_count=mysql_num_rows($result);
					for($i=0;$i<$row_count;$i++){
