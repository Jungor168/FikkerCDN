...
function FikCdn_SyncRCache(){
	...
	if (nodeSelect.length==0 || nodeSelect2.length==0){ 
		var boxURL="msg.php?1.9&msg=目前没有服务器，不能同步拒绝缓存规则。";
		showMSGBOX('',350,100,BT,BL,120,boxURL,'操作提示:');	
	  	return false;
	}
	
	var postURL="./ajax_cache_rules.php?mod=rcache&action=sync";
	var postStr="node_id="+UrlEncode(nodeSelect)+"&node_id2=" + UrlEncode(nodeSelect2);
	AjaxBasePost("rcache","sync","POST",postURL,postStr);
}

function FikCdn_SyncRCacheResult(sResponse){
	var json = eval("("+sResponse+")");
	if(json["Return"]=="True"){
		var boxURL="msg.php?6.9";
		showMSGBOX('',350,100,BT,BL,120,boxURL,'操作提示:');
	}else{
...
<table width="500" border="0" align="center" cellpadding="0" cellspacing="0" class="normal">
  ...
  <tr>
  	...
  	<td width="220" class="objTitle" style="text-align:left;">将此服务器的所有拒绝缓存规则：</td>
  </tr>
  <tr>
 ...
 ?>
  ...
  <tr>
    <td colspan="2" style="padding-left:120px;">
	    <input name="btnSyncRCache"  type="submit" style="width:105px;height:28px" id="btnSyncRCache" value="确定" style="cursor:pointer;" onclick="FikCdn_SyncRCache();" />
	</td>
  </tr>
</table>
