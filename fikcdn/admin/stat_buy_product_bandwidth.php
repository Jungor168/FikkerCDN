...
function getCrashReportStatData(buy_id,timeval)
{
	...
	var postUrl = "request_stat_data.php?mod=product&action=bandwidth&buy_id="+buy_id+"&timeval="+timeval;
...
function selectProduct()
{
	...
	window.location.href="stat_buy_product_bandwidth.php?buy_id="+buy_id+"&timeval="+nBandwidthDateSelect+"&date2="+nDateSelect2;
}
    ...
    <td valign="top" background="../images/content-bg.gif">
	   <table width="100%" height="31" border="0" cellpadding="0" cellspacing="0" class="left_topbg" id="table2" >
			<tr height="31" class="TabToolTitle">
				<td height="31" width="85"><a href="stat_buy_product_bandwidth.php?buy_id=<?php  echo $buy_id; ?>"><span class="title_bt">实时带宽</span></a></td>
...
<?php
	...
	if($db_link)
	{
		...
		$sql = "SELECT max(bandwidth_down),max(bandwidth_up),avg(bandwidth_down),avg(bandwidth_up) FROM domain_stat_product_bandwidth WHERE buy_id='$buy_id' AND time>=$timeval2";
		$result2 = mysql_query($sql,$db_link);
		if($result2 && mysql_num_rows($result2)>0)
		{
		...
		// 计算最大带宽
		$timeval2 = $timenow-24*60*60*3;
		$sql = "SELECT max(bandwidth_down),max(bandwidth_up) FROM domain_stat_product_bandwidth WHERE buy_id='$buy_id' AND time>=$timeval2";
		$result2 = mysql_query($sql,$db_link);
		if($result2 && mysql_num_rows($result2)>0)
		{
 ...
 ?>
	...
	<table width="800" border="0" class="bottom_btn">
			<tr height="30" bgcolor="#FFFFE6">
			<td>
			<span class="input_tips_txt3">套餐实时带宽详细统计数据 - <?php echo $show_this_name; ?></span>
...
<?php				
			if($db_link){
				...
				$sql = "SELECT * FROM domain_stat_product_bandwidth where buy_id='$buy_id' AND time>='$timeval1' AND time<'$timeval2' ORDER BY time DESC"; 
				$result = mysql_query($sql,$db_link);
				if($result){
					$row_count=mysql_num_rows($result);
					for($i=0;$i<$row_count;$i++){
...
getCrashReportStatData(<?php echo $buy_id; ?>,nCurBandwidthDateSelect);
</script>
<?php
