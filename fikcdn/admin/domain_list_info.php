...
function selectPage(obj){
	...
	window.location.href="domain_list_info.php?page="+pagesSelect+"&action=jump"+"&buy_id="+txtBuyId+"&keyword="+UrlEncode(txtKeyword)+"&type="+searchSelect+"&selectOrder="+selectOrder;
}
...
function FikHost_Search(){
	...
	window.location.href="domain_list_info.php?page=1"+"&action=jump"+"&buy_id="+txtBuyId+"&keyword="+UrlEncode(txtKeyword)+"&type="+searchSelect;
...
function selectGroup(){
	...
	var txtBuyId		 =document.getElementById("grpSelect").value;
	var selectOrder =document.getElementById("selectOrder").value;
	window.location.href="domain_list_info.php?page=1"+"&action=jump"+"&buy_id="+txtBuyId+"&keyword="+UrlEncode(txtKeyword)+"&type="+searchSelect+"&selectOrder="+selectOrder;
}
    ...
    <td valign="top" background="../images/content-bg.gif">
	   <table width="100%" height="31" border="0" cellpadding="0" cellspacing="0" class="left_topbg" id="table2" >
			<tr height="31" class="TabToolTitle">
				<td height="31" width="85"><a href="domain_list.php?buy_id=<?php echo $buy_id.'&page='.$nPage.'&action='.$action.'&type='.$sType.'&keyword='.$sKeyword.'&selectOrder='.$selectoder; ?>"><span class="title_bt_active">域名列表</span></a></td>
				<td height="31" width="85"><a href="domain_list_info.php?buy_id=<?php echo $buy_id.'&page='.$nPage.'&action='.$action.'&type='.$sType.'&keyword='.$sKeyword.'&selectOrder='.$selectoder; ?>"><span class="title_bt">更多信息</span></a></td>
	  ...
	  <td valign="top">
			...
			<td><div class="div_search_title">
				...
				<select id="selectOrder" name="selectOrder" style="width:240px" onChange="selectGroup(this)">
					<option value="down_dataflow_total" <?php if($selectoder=="down_dataflow_total") echo 'selected="selected"'; ?>>累计流量</option>
		...
		<div id="div_search_result">
		  <table width="800" border="0" class="dataintable" id="domain_table">
			<tr id="tr_domain_title">
				...
				<th align="right" width="160">域名 ICP 备案号</th>						
				<th align="center" width="260">备注</th>											
...
?>
		 ...
		 <table width="800" border="0" class="disc">
			...
			<td bgcolor="#FFFFE6"><div class="div_page_bar"> 记录总数：<?php echo $total_host;?>条&nbsp;&nbsp;&nbsp;当前第<?php echo $nPage; ?>页|共<?php echo $total_pages; ?>页&nbsp;&nbsp;&nbsp;跳转
				...
				页&nbsp;&nbsp;&nbsp;&nbsp;<a href="domain_list_info.php?page=1&action=first&buy_id=<?php echo $buy_id.'&keyword='.$sKeyword.'&type='.$sType.'&selectOrder='.$selectoder; ?>">首页</a>&nbsp;&nbsp;
				<a href="domain_list_info.php?page=<?php echo $pageup; ?>&action=pageup&buy_id=<?php echo $buy_id.'&keyword='.$sKeyword.'&type='.$sType.'&selectOrder='.$selectoder; ?>">上一页</a>&nbsp;&nbsp;				
				<a href="domain_list_info.php?page=<?php echo $pagedown; ?>&action=pagedown&buy_id=<?php echo $buy_id.'&keyword='.$sKeyword.'&type='.$sType.'&selectOrder='.$selectoder; ?>">下一页</a>&nbsp;&nbsp;
				<a href="domain_list_info.php?page=<?php echo $total_pages; ?>&action=tail&buy_id=<?php echo $buy_id.'&keyword='.$sKeyword.'&type='.$sType.'&selectOrder='.$selectoder; ?>">尾页 </a></div></td>
			</tr>
		</table>	
